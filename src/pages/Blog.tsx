...
export default function Blog() {
  ...
  useSEO({
    ...
    description: 'Explore the Sit With Me journal – stories on intentional living, effortless fashion, travel destinations, and wellness. New stories every week.',
    canonical: '/blog',
    ...
    schema: {
      ...
      description: 'Stories on intentional living, fashion, travel, and wellness.',
      publisher: {
        '@type': 'Organization',
        name: 'Sit With Me',
          ...
          {filteredPosts.map((post) => (
            <article key={post.id}>
              <Link to={`/blog/${post.slug}`} className="group">
                ...
                <time className="text-[10px] uppercase tracking-widest text-ink/40" dateTime={post.date}>{post.date}</time>
              </Link>
            </article>
          ))}
        </div>
