...
// Default SEO while post is loading
function LoadingSEO() {
  useSEO({
    title: 'Loading Story... | Sit With Me Journal',
    description: 'Reading a story from the Sit With Me lifestyle journal.',
  });
  return null;
}
  ...
  useSEO({
    ...
    schema: {
      ...
      publisher: {
        ...
        logo: {
          '@type': 'ImageObject',
          url: 'https://sitwithme.com/logo.png'
        }
      },
      articleSection: post.category,
      mainEntityOfPage: {
        '@type': 'WebPage',
        '@id': `https://sitwithme.com/blog/${post.slug}`
      }
    }
  });
  return null;
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <header className="mb-16">
            ...
            <div className="flex items-center justify-between border-y border-ink/5 py-6">
              <time className="text-sm italic font-serif text-ink/60" dateTime={post.date}>{post.date}</time>
              
              <div className="flex items-center space-x-6">
                <span className="text-[10px] uppercase tracking-widest text-ink/40">Share</span>
                <div className="flex items-center space-x-4">
                  <button onClick={shareOnTwitter} className="text-ink/60 hover:text-ink transition-colors" aria-label="Share on Twitter">
                    <Twitter size={18} />
                  </button>
                  <button onClick={shareOnFacebook} className="text-ink/60 hover:text-ink transition-colors" aria-label="Share on Facebook">
                    <Facebook size={18} />
                  </button>
                  <button onClick={shareOnPinterest} className="text-ink/60 hover:text-ink transition-colors" aria-label="Share on Pinterest">
                    <Share2 size={18} />
                  </button>
                </div>
              </div>
          ...
          <footer className="mt-24 pt-12 border-t border-ink/5 flex justify-center">
            <div className="flex items-center space-x-8">
              ...
              <div className="flex items-center space-x-6">
                <button onClick={shareOnTwitter} aria-label="Share on Twitter" className="hover:opacity-50 transition-opacity"><Twitter size={20} /></button>
                <button onClick={shareOnFacebook} aria-label="Share on Facebook" className="hover:opacity-50 transition-opacity"><Facebook size={20} /></button>
                <button onClick={shareOnPinterest} aria-label="Share on Pinterest" className="hover:opacity-50 transition-opacity"><Share2 size={20} /></button>
              </div>
            </div>
          </footer>
        </div>
